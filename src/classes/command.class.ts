...
export abstract class Command {
  ...
  public static getCommandName(message: Message): string {
    return message.content.match(Command.REGEX_NAME)?.[1]?.toLowerCase() ?? 'none';
  }

  constructor(message: Message) {
    const { command, args, query, user, options } = this.parseMessage(message);
  ...
  private log(): void {
    ...
    LogUtils.log(author.tag, content);
  }
}
